...
filesToTranslate.forEach(filePath => {
  ...
  const targetFile = path.join(i18nDir, filePath.replace('.mdx', '.json').replace('.md', '.json'));
  ...
  try {
    ...
    if (data.title) {
      const englishTitle = translations[data.title] || data.title;
      translation.title = {
        message: englishTitle,
        description: `The title for ${filePath}`
      };
    }
    
    if (data.sidebar_label) {
      const englishLabel = translations[data.sidebar_label] || data.sidebar_label;
      translation['sidebar_label'] = {
        message: englishLabel,
        description: `The label for ${filePath} in the sidebar`
      };
    }
    ...
    if (data.description) {
      translation.description = {
        ...
        description: `The description for ${filePath}`
      };
    }
    ...
    // 写入翻译文件
    fs.writeFileSync(targetFile, JSON.stringify(translation, null, 2), 'utf8');
