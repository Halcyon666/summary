...
sidebar_position: 1
description: 'Comprehensive Git command reference: config, log, diff, rebase, merge,
  stash'
tags:
- development
- tools
...
### 在IDEA中PS Terminal中使用`git rebase -i`时无法退出

import MyHighlight from "@site/src/components/MyHighLight";

`git rebase -i` 强制要求必须使用<MyHighlight color="white" background="red">git rebase</MyHighlight> 来执行。
