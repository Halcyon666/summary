...
sidebar_position: 10
description: 1. xml配置方式 proxy-target-class=true：强制使用 CGLib 代理 proxy-target-class=false：Spring
  会根据情况自动选择代理方式. import MyHighlight from "@site/src/components/MyHighLi
tags:
- algorithm
- computer-science
- data-structures
...
<MyHighlight color="yellow">两种代理方式的区别：</MyHighlight><br/>
<MyHighlight color="yellow">JDK 动态代理：只能代理实现了接口的类,基于接口创建代理对象,性能较好，但有接口限制</MyHighlight><br/>
<MyHighlight color="yellow">CGLib 代理：可以代理没有接口的类,通过继承目标类创建代理对象,功能更强大，但性能相对较低</MyHighlight>
...
```
 ...
 方法二：使用ObjectFactory，[请看这里](https://halcyon666.github.io/summary/Java/Spring/IoC%20Inject2/#通过objectfactory)
...
<details>
    <summary>static @Bean 示例</summary>
```java
@Configuration
...
- <MyHighLight color="red">不能使用配置类字段注入</MyHighLight>。
- 默认仍是<MyHighLight color="red">单例</MyHighLight>，Spring会缓存对象，下次获取直接复用。
- 静态方法减少CGLIB开销。
- 只能使用构造方法或者`ApplicationContext.getBean(...)`来获取。

:::tip
适用场景：
早基础设施Bean 如PropetySourcePlaceholerConfigurer<br/>
避免循环依赖<br/>
